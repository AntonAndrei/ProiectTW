 ...
 <head>
  ...
  ?>
   ...
   <span>
    ...
    <h2 class="cTitle"> FOOD
    <h3 class="cText"> Track all your food related spendings.
	                   See if maybe you are spending too much money on
					   a certain product.</h3>
    </h2>
    </div>
		...
		$Group = strrev ($Group);
			...
			if($grID == 0)
			{
				$sql = "SELECT name, user_id, cost, date, obs FROM spendings 
						WHERE group_id = $grID AND category = 'food'
			...
			else
			{
				...
				$sql = "SELECT name, user_id, cost, date, obs FROM spendings 
						WHERE group_id = $grID AND category = 'food' ORDER BY date DESC;";
				$result = $con->query($sql);
				if ($result->num_rows > 0)
				{
					$Total = 0.00;
